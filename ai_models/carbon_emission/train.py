"""
Model 14: Carbon Emission Prediction
...
DATA = os.path.join(BASE, '../datasets/carbon_emission.csv')
df = pd.read_csv(DATA)

cat_cols = ['aircraft_type','engine_type']
encoders = {}
for col in cat_cols:
    le = LabelEncoder()
...
FEATURES = ['aircraft_type','distance_km','passengers','fuel_kg','load_factor_pct','engine_type']
TARGET = 'co2_kg'
...
with open(os.path.join(BASE, 'carbon_model.pkl'), 'wb') as f:
...
print(f"[14] Carbon Emission  | Accuracy: {accuracy}%  MAE: {mae:.0f} kg  R2: {r2:.3f}")
