"""
Model 11: Weather Risk Prediction
...
DATA = os.path.join(BASE, '../datasets/weather_risk.csv')
df = pd.read_csv(DATA)

cat_cols = ['airport','fog_level','season']
encoders = {}
for col in cat_cols:
    le = LabelEncoder()
...
FEATURES = ['airport','wind_speed_kmh','visibility_km','precipitation_mm',
            'temperature_c','humidity_pct','lightning_risk','fog_level','season']
TARGET = 'risk_level'
...
with open(os.path.join(BASE, 'weather_model.pkl'), 'wb') as f:
...
print(f"[11] Weather Risk Prediction  | Accuracy: {acc}%")
